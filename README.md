...
Do not refresh SwiftBar to update this. Re-run apply (`?v11`):

```
cd /Users/andrew/.config/swiftbar/plugins
curl -fsSL "https://raw.githubusercontent.com/presempathy-awb/swiftbar-one-screen-bottom/main/macos-install.sh?v11" -o apply.sh
chmod +x apply.sh
./apply.sh --apply
```

Apply prints the overlay log. You want `overlay rev=v11`, `topHalf=false`, and `y=` equal to `screenMinY`. `SwiftBar is not running.`
